...
int main(int argc, char *argv[]) {
	...
	//Lectura archivo
	string verticesTexto = lectura("../../polygons.txt");
	...
	do{
		...
		for (list<vector<float>>::iterator it = vertices.begin(); it != vertices.end(); it++) {
			...
			if (aux % 2 == 0) {
				...
				//cout << "C (" << it->at(0) << ", " << it->at(1) << ", " << it->at(2) << ")" << endl;
			}else {
				glVertex3f(it->at(0), it->at(1), it->at(2));
				//cout << "V (" << it->at(0) << ", " << it->at(1) << ", " << it->at(2) << ")" << endl;
			}

			if (!abre && (aux+1) % 6 == 0) {
				cout << "Cierro: " << aux << endl;
				glEnd();
				abre = true;
			}
		...
		while (SDL_PollEvent(&evento)){
			...
			case SDL_KEYDOWN:
				...
				case SDLK_ESCAPE:
					...
					break;
				case SDLK_SPACE:
					verticesTexto = lectura("../../polygons.txt");
					vertices = cargarVertices(verticesTexto);
					break;
				case SDLK_l:
...
vector<float> string2vectorFloat(string lista) {
	...
	while (lista[i] != '\n') {
		if (int(lista[i]) == 46 || int(lista[i]) == 45 || int(lista[i]) > 47 && int(lista[i]) < 58) {
			...
			cout <<"Coordenada: " << aux << " ~ " << stof(aux) << endl;
